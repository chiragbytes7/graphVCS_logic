...
class VersionControl:
    ...
    def merge_branches(self, source_branch, target_branch, user_id, message, directory_delete):
        ...
        with self.driver.session() as session:
            ...
            hash_for_source_dir, hash_for_source_files = self.fetch_version_hash(head_for_source_id)
            ...
            if(hash_for_ancestor_dir == hash_for_target_dir and hash_for_ancestor_dir != hash_for_source_dir):            # this means that only source has changed, fast forward merge
                ...
                self.merge_commit(head_for_source_id, head_for_target_id, user_id)
                ...
                self.merge_commit(head_for_source_id, head_for_target_id, user_id)      
                print("new merge commit created")    
            elif(hash_for_ancestor_dir != hash_for_target_dir and hash_for_ancestor_dir != hash_for_source_dir and hash_for_source_dir != hash_for_target_dir):
                print("conflicting versions")                                                                             # conflict errors occur here 
                print(f"conflict between versions {head_for_source_id} and {head_for_target_id}")
                
            
    def merge_commit(self, commit1, commit2, user_id):
        last_commit_id = self.read_latest_commit()
        merge_commit_id = last_commit_id + 1
        
        with self.driver.session() as session:
            result = session.run("""
                                 MERGE (commit:COMMIT {id: $merge_commit_id})
                                 MATCH (a:Commit {id : $source_id}), (b:Commit {id : $merge_commit_id}), (c:Commit {id : $target_id}), (u:User {id : $user_id})
                                 CREATE (b)-[:PARENT]->(a), (b)-[:PARENT]->(c)
                                
                                 """, merge_commit_id=merge_commit_id, source_id = commit1, target_id = commit2, user_id = user_id)
...
def main():
    ...
    args = parse_args()
    vc = VersionControl(uri="bolt://localhost:7689", user="neo4j", password="Bangalore@01")
